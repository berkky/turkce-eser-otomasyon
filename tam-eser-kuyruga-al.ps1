...
Write-Host "--- TAM ESER KUYRUGA AL (Adim 31) ---"
Write-Host "Eser ID: ESER-$("{0:D5}" -f $EserId)"
Write-Host "Onayli: $Onayli"
Write-Host "Not: Gercek TTS uretimi baslatilmaz."
Write-Host ""
...
$argsList = @("$EserId")
if ($Onayli) { $argsList += "-Onayli" }
$argsJoined = $argsList -join " "
& $Maven -q "-Dexec.mainClass=TamEserKuyrugaAlApp" "-Dexec.args=$argsJoined" exec:java
exit $LASTEXITCODE

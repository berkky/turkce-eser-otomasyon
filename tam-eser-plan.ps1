...
Write-Host "--- TAM ESER URETIM PLANI (Adim 31) ---"
Write-Host "Eser ID: ESER-$("{0:D5}" -f $EserId)"
Write-Host "Not: Gercek TTS uretimi yapilmaz. Yalnizca plan JSON uretilir."
Write-Host "Uyari: Onay olmadan uretim baslatilmaz."
Write-Host ""

& $Maven -q -DskipTests compile
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $Maven -q "-Dexec.mainClass=TamEserPlanApp" "-Dexec.args=$EserId" exec:java
exit $LASTEXITCODE
